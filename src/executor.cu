...
void Executor::register_queue(string model_name, int queue_id, PhysicalOpQueue queue) {
  cudaStream_t s;
  cudaStreamCreate(&s);
  ...
  while (true) {
    ...
    for (ExecutorCtx& ctx_struct: executor_queues) {
      while (ctx_struct.queue->try_dequeue(op)) {
        ...
        events_registrar->insert(ctx_struct.model_name, ctx_struct.queue_id, events);
      }
    }
  }
}