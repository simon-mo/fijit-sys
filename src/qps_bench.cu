...
int main(int argc, char *argv[]) {
  ...
  // Preloading kernels for constant resource allocations
  ...
  for (int l = 0; l < num_query; ++l) {
    ...
    CHECK_CUDA(cudaEventCreate(&ctx->dispatch_time));
    CHECK_CUDA(cudaEventCreate(&ctx->start_time));
    ...
    CHECK_CUDA(cudaEventRecord(ctx->end_time, s));
  ...
  for (int q = 0; q < num_query; ++q) {
    ...
    cudaEvent_t dispatch_time = ctx.dispatch_time;
    cudaEvent_t start_time = ctx.start_time;
    cudaEvent_t complete_time = ctx.end_time;
    ...
    cudaEventDestroy(dispatch_time);
